 ...
 */
export default function useCacheParts(ids: string[] | [], { skip, reCache }: { skip?: boolean, reCache?: never } | { skip?: never, reCache?: boolean } = {}){
  ...
  const info = ids.length > 0 && ids.every(id => partsDB.ids.includes(id)) ? ids.map(id => partsDB.entities[id]).filter(id => id != undefined) as PCPartInfo[] : undefined


  // query to get missing data if not in store cache:
  const query = useFetchPartsQuery(!skip && !info ? { ids: ids.filter(id => !partsDB.ids.includes(id)), oems: [], types: [] } : skipToken)
  const { data, refetch } = query

  
  // if store cache miss, add query data from RTK Query cache or database responce to store cache:
  useEffect(() => {
    dbgLog("useCacheParts.ts", ["useCacheParts", "useEffect(,[data])"], "preCached", preCached, "info", info, "query", query, "partsDB", partsDB, "ref info === data", info === data)
