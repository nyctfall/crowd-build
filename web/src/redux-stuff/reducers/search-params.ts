...
export const partSearchParamsSlice = createSlice({
  ...
  reducers: {
    ...
    setMinMSRPs: (state, { payload: minPrice }: PayloadAction<PCPartSearchURI["minPriceFilter"] | string>) => {
      ...
      if (priceChecker.minPriceFilter === minPrice || priceChecker.minPriceFilter?.eq?.(minPrice as Exclude<PCPartSearchURI["minPriceFilter"],undefined> | string)) state.minPrice = priceChecker.minPriceFilter
    },
    unsetMinMSRP: (state) => {
    ...
    setMaxMSRPs: (state, { payload: maxPrice }: PayloadAction<PCPartSearchURI["maxPriceFilter"] | string>) => {
      ...
      // if price is undefined, or coerced sucessfully:
      if (priceChecker.maxPriceFilter === maxPrice || priceChecker.maxPriceFilter?.eq?.(maxPrice as Exclude<PCPartSearchURI["maxPriceFilter"],undefined> | string)) state.maxPrice = priceChecker.maxPriceFilter
    },
    unsetMaxMSRP: (state) => {
