...
export class ArrayWithTypes<T> {
  ...
  readonly #isTypeCoercible = (arg: any): boolean => {
    if (
      (typeof this.#type === "string" && typeof arg === this.#type) ||
      (this.#type &&
        typeof this.#type === "object" &&
        (typeof (this.#type as any)[Symbol.hasInstance] === "function" || typeof this.#type === "function") &&
        arg instanceof (this.#type as any))
    )
      return true
    return false
  }
  ...
  constructor({ type, typeCoercer, isTypeCoercible, initArray, seal, freeze }: ArrayWithTypesOpts<T>) {
    ...
    if (Array.isArray(initArray)) {
      this.#array = initArray.filter(v => this.#isTypeCoercible(v)).map(coercibleV => this.#typeCoerce(coercibleV) as T)
      // static Array size:
  ...
  splice(start: number, deleteCount?: number, ...items: T[]) {
    const itemsT = items.filter(v => this.#isTypeCoercible(v)).map(coercibleV => this.#typeCoerce(coercibleV) as T)
...
export class ArrayTypeSafeMethods<T> extends Array<T> {
  ...
  constructor({ type, coercer, verifier, initArray, seal, freeze }: ArrayTypeConfig<T>) {
    ...
    if (Array.isArray(initArray)) {
      this.push(...initArray.filter(v => this.#verifier(v)).map(coercibleV => this.#coercer(coercibleV) as T))
      // static Array size:
   ...
   */
  splice(start: number, deleteCount?: number, ...items: T[]) {
    const itemsT = items.filter(v => this.#verifier(v)).map(coercibleV => this.#coercer(coercibleV) as T)
...
export function ArrayTypeSafeProxy<T>(config: ArrayTypeConfig<T>) {
  ...
  return new Proxy<ArrayTypeSafeMethods<T>>(internalArray, {
    set(
      target: ArrayTypeSafeMethods<T>,
      property: (string | symbol) & keyof ArrayTypeSafeMethods<T>,
      value: ArrayTypeSafeMethods<T>[keyof ArrayTypeSafeMethods<T>],
      receiver: any
    ): boolean {
    ...
    get(
      ...
      property: (string | symbol) & keyof ArrayTypeSafeMethods<T>,
      receiver: any
    ): ArrayTypeSafeMethods<T>[keyof ArrayTypeSafeMethods<T>] {
